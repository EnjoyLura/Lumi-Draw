...
test("builds 640px Q85 WebP images for work cards", () => {
  const url = service().readResponsiveImageUrl(
    "https://bucket.oss.example.com/uploads/work/image.png",
    "public"
  );

  assert.match(decodeURIComponent(url), /image\/resize,w_640\/quality,Q_85\/format,webp/);
});

test("builds non-cropping 1440px Q92 WebP images for work detail previews", () => {
  const url = service().readDetailPreviewImageUrl(
    "https://bucket.oss.example.com/uploads/work/image.png",
    "public"
  );

  assert.match(decodeURIComponent(url), /image\/resize,m_lfit,w_1440,h_1440\/quality,Q_92\/format,webp/);
});

test("builds non-cropping 2048px Q95 WebP images for full-screen previews", () => {
  const url = service().readFullscreenImageUrl(
    "https://bucket.oss.example.com/uploads/work/image.png",
    "public"
  );
