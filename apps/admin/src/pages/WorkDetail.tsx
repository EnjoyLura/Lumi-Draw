...
function EditWorkInfoForm({ work }: { work: AdminWork }) {
  ...
  const save = () => {
    work.title = title;
    work.desc = desc;
    work.tags = tags.split(/[、,，]/).map((s) => s.trim()).filter(Boolean);
    closeSheet();
    toast("已保存作品信息");
  };
...
function TakedownForm({ work }: { work: AdminWork }) {
  const { closeSheet, toast } = useNav();
  const doTakedown = () => {
    work.status = "已下架";
    work.featured = false;
  ...
  return (
    <>
      ...
      <div style={FOOT_STYLE}>
        <button className="btn btn-ghost btn-block" onClick={closeSheet}>取消</button>
        <button className="btn btn-danger btn-block" onClick={doTakedown}>确认下架</button>
      </div>
    </>
  );
}
...
export function WorkDetail({ param }: { param?: string }) {
  ...
  const u = USERS.find((x) => x.id === w.userId) ?? USERS[0];
  const tagList = w.tags && w.tags.length ? w.tags : [w.style];

  const toggle = (key: "featured" | "recommend") => {
    w[key] = !w[key];
    toast(w[key] ? "已开启" : "已关闭");
  };
  ...
  const del = () => confirmDlg("删除作品", "删除后不可恢复，确定删除该作品吗？", () => {
  ...
  return (
    <>
      ...
      <Sec title="标题与描述" more="编辑" onMore={() => openSheet("编辑作品信息", <EditWorkInfoForm work={w} />)} />
      ...
      <div className="actionbar">
        {w.status === "已下架" ? (
          ...
          <button className="btn btn-danger btn-block" onClick={() => openSheet("下架作品", <TakedownForm work={w} />)}><i className="ri-eye-off-line" />下架</button>
        )}
