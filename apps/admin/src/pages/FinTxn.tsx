...
import { apiGetPaymentOrdersPage } from "../data/api";
...
export function FinTxn() {
  const { openSheet } = useNav(); const { useMock } = useAdminSession(); const [type, setType] = useState("全部"); const [status, setStatus] = useState("全部"); const [page, setPage] = useState(1);
  const paymentType = type === "充值" ? "recharge" : type === "会员" ? "membership" : undefined;
  const paymentStatus = status === "成功" ? "paid" : status === "待支付" ? "pending" : status === "失败" ? "failed" : status === "已退款" ? "refunded" : undefined;
  const paymentQuery = useQuery({
    queryKey: ["admin", "payment-orders", page, paymentType, paymentStatus],
    queryFn: () => apiGetPaymentOrdersPage({ page, pageSize: 20, type: paymentType, status: paymentStatus }),
    enabled: !useMock,
    placeholderData: (previous) => previous
  });
  const all = useMock ? getTransactions() : paymentQuery.data?.items ?? [];
  const list = useMock ? all.filter((item) => (type === "全部" || item.type === type) && (status === "全部" || item.status === status)) : all;
  const total = useMock ? list.length : paymentQuery.data?.total ?? 0;
  const changeType = (value: string) => { setType(value); setPage(1); };
  const changeStatus = (value: string) => { setStatus(value); setPage(1); };
  return <div className="lumi-admin-page"><Card className="lumi-table-card" title="交易记录" extra={<Typography.Text type="secondary">共 {total} 条</Typography.Text>}><div className="lumi-table-toolbar"><Space wrap><Typography.Text type="secondary">类型</Typography.Text><Segmented options={["全部", "充值", "会员", "消费", "退款", "签到"]} value={type} onChange={(value) => changeType(String(value))} /></Space><Space wrap><Typography.Text type="secondary">状态</Typography.Text><Segmented options={["全部", "成功", "待支付", "失败", "已退款"]} value={status} onChange={(value) => changeStatus(String(value))} /></Space></div><Table<AdminTxn> rowKey="id" loading={paymentQuery.isFetching} dataSource={list} pagination={{ current: page, pageSize: 20, total, showSizeChanger: false, onChange: setPage }} locale={{ emptyText: paymentQuery.error instanceof Error ? paymentQuery.error.message : "暂无交易记录" }} columns={[
