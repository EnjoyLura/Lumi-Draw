...
function ModelForm({ id, onSaved }: { id: string; onSaved: () => void }) {
...
export function OpsModel() {
  ...
  const openForm = (id: string) => openSheet(id ? "编辑模型" : "新增模型", <ModelForm id={id} onSaved={refresh} />);
  const toggle = (m: AdminModel) => { m.on = !m.on; refresh(); toast(m.on ? "模型已上线" : "模型已下线"); };
