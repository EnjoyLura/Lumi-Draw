...
import LumiPageHeader from "./LumiPageHeader.vue";
...
function syncStandaloneDetail() {
  ...
  if (!work.value || !user.value || consumeWorkDetailStale(workId.value)) {
    isInitialContentReady.value = true;
    void loadDetail();
  }
}
...
async function refreshEmbeddedDetail() {
  if (!props.embedded || !props.open || !work.value || useMockData.value) return;
